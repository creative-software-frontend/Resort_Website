...
const Footer: React.FC = () => {
  ...
  return (
    // 1. FIXED: Set the solid dark background color here on the root footer element
    <footer
      id="contact"
      style={{ background: '#08081a' }}
      className="relative overflow-hidden text-gray-300 border-t border-gray-800"
    >

      {/* 2. BACKGROUND IMAGE LAYER: Positioned absolutely right under the base color */}
      <div className="absolute inset-0 pointer-events-none select-none z-0">
        <img
          src={footerBg}
          alt="Goldsands Group resort aerial view"
          className="w-full h-full object-cover object-center"
          // Enhanced image visibility styling layout parameters
          style={{
            filter: 'brightness(0.5) saturate(0.8) hue-rotate(200deg)',
            opacity: 0.4
          }}
        />
        {/* Soft linear dark gradient to allow seamless text contrast balancing */}
        <div
          className="absolute inset-0"
          style={{ background: 'linear-gradient(180deg, rgba(8,8,26,0.3) 0%, rgba(8,8,26,0.9) 100%)' }}
        />
      </div>

      {/* 3. CONTENT LAYER: Cleared background style properties and forced stack order on top */}
      <div className="relative z-10 gs-container py-16 bg-transparent">
